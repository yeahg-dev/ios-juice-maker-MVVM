...
### Observable 아이템를 효율적 공유하는 방법 `share`
...
`retry`연산자는 에러가 방출되면 에러를 내보낸 후 스트림을 dispose하고 **다시 `subscribe`를 호출**합니다.  에러가 방출되는 스트림에서 걔속해서 사용자 이벤트를 받기위해 `retry`를 사용해 에러 핸들링을 해보았습니다.
